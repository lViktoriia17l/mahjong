...
class MainMenu(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(self, text="STM32 Mahjong Console", font=("Arial", 28, "bold"), bg="#f0f0f0", fg="#333").pack(
            pady=(100, 10))
        tk.Label(self, text="Select your device port to begin", font=("Arial", 12), bg="#f0f0f0", fg="#666").pack(
            pady=(0, 30))
        ...
        self.combo_ports = ttk.Combobox(frame_controls, textvariable=self.port_var, width=15, state="readonly",
                                        font=("Arial", 11))
        self.combo_ports.grid(row=0, column=1, padx=10)

        btn_refresh = tk.Button(frame_controls, text="↻", command=self.refresh_ports, font=("Arial", 12, "bold"),
                                width=3)
...
class GameInterface(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- ОНОВЛЕНО: Кнопка Reset замість New Game ---
        tk.Button(toolbar, text="🔄 Reset", command=self.send_reset_command, bg="#f44336", fg="white",
    ...
    def send_reset_command(self):
        """ОНОВЛЕНО: Спочатку скидає сесію (Reset), а потім запитує нове поле (Start)"""
        if not self.controller.uart.is_open: return
        self.log("Requesting Game Reset...")
        ...
        if len(resp) == 3 and resp[1] == 0x00:
            self.log("Reset Successful! Fetching new layout...")
    ...
    def draw_pyramid(self, data):
        ...
        def draw_tile(index, grid_x, grid_y, layer_z, offset_x_tiles, offset_y_tiles):
            ...
            self.canvas.create_rectangle(x + SHADOW_OFFSET, y + SHADOW_OFFSET, x + TILE_W + SHADOW_OFFSET,
                                         y + TILE_H + SHADOW_OFFSET, fill="#1a1a1a", outline="")
            self.canvas.create_rectangle(x, y, x + TILE_W, y + TILE_H, fill="#f0f0f0", outline=border_color,
                                         width=border_width)
            self.canvas.create_rectangle(x + 2, y + 2, x + TILE_W - 2, y + TILE_H - 2, fill=color, outline="")
            self.canvas.create_text(x + TILE_W / 2, y + TILE_H / 2, text=str(value), font=("Arial", 16, "bold"),
                                    fill="black")
...
class MahjongApp(tk.Tk):
    ...
    def show_game(self):
        ...
        # --- ОНОВЛЕНО: Тепер при підключенні відправляється RESET замість START ---
        self.after(200, self.game_view.send_reset_command)
    ...
    def trigger_auto_reconnect(self):
        ...
        self.show_menu()
        self.menu_view.lbl_status.config(text=f"Connection lost! Auto-reconnecting to {self.reconnect_port}...",
                                         fg="red")
        messagebox.showwarning("Connection Lost",
                               f"No response from STM32 on {self.reconnect_port}.\n\nPlease ensure it is plugged in.\nThe app will attempt to automatically reconnect.")
